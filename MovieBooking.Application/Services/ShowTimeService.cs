...
namespace MovieBooking.Application.Services
{
    ...
    public class ShowTimeService : IShowTimeService
    {
        ...
        public ShowTimeService(IShowTimeRepository showTimeRepository, ITheatreRepository theatreRepository)
        {
        ...
        {
            var slots = await TheatreRepository.GetTimeSlotsByTheatreAsync(createShowTimeDto.TheatreId);
            if (!slots.Any())
            ...
            foreach (var slot in slots)
            {
                ...
                bool conflict = await ShowTimeRepository.ShowTimeConflictExistsAsync(createShowTimeDto.ScreenId, start, end);
                if (conflict)
        ...
        {
            var showTime = await ShowTimeRepository.GetShowTimeByIdAsync(showTimeId);
            var slots = await TheatreRepository.GetTimeSlotsByTheatreAsync(showTime.TheatreId);
            ...
            var conflict = await ShowTimeRepository.ShowTimeConflictExistsAsync(showTime.ScreenId, start, end);
            if (conflict)
            {
                var conflictingShowTime = await ShowTimeRepository.GetShowTimeByIdAsync(showTimeId);
